...
(defn draw-offset-arrow [[a b][c d]] ; [a b] is the tail, [c d] offsets
  (let [headx (+ a c) 
        heady (+ b d)]
    (line a b headx heady)
    (line headx heady
          (+ headx (* -0.086 c) (* -0.05 d))
          (+ heady (*  0.05 c) (* -0.086 d)))
    (line headx heady
          (+ headx (* -0.086 c) (*  0.05 d))
          (+ heady (* -0.05 c) (* -0.086 d)))))
...
;; This arrow points roughly northeast (3 miles east and 4 miles
;; north, in fact)
...
;; And this one points more southeast (4 miles east and -3 north, or 3
;; miles south)
...
;; This addition rule looks very simple in terms of coordinates, and
;; geometrically it's very simple too.

;; You draw one arrow (it's already there actually)

(do (ink yellow) (draw-arrow arrow1))

;; And then you draw the second one, but starting at the head of the
;; first one:
...
;; Now we're going to do something slightly weirder, but in the same
;; spirit.
...
;; You might be able to see that there's a sense here of multiplying
;; things, and of exchanging x coordinates and y coordinates, and
;; taking pluses to minuses.

;; You might notice above that arrow2 [4, -3] is arrow1 [3,4] rotated
;; by 90 degrees, and that to get one from the other (3,4) -> (-4, 3),
;; we exchange x and y and swap +4 for -4.

;; In fact, we've encoded the idea of 'rotating and zooming' into our
;; multiplication law.
...
;; The two arrows we started with were five long, but the result of
;; multiplying them is 25 long.  Of course, we could have guessed
;; that. 500% zoom followed by 500% zoom is 2500% zoom!
...
;; Arrow 3 represents a very small turn anticlockwise, and a very
;; small zoom
